...
class ReportController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        if(request()->has('course') && request()->has('type')) {
            ...
            if(!is_null($course)) {
                $intro_remarks = Remark::where(['user_id' => $user->id, 'course_id' => $course->course_id, 'type' => $type])->whereNotNull('assessment_grade')->select('completion_grade','assessment_grade')->get();
